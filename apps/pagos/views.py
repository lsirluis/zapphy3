...
from apps.pagos.models import Recibo , Detalle, Noticia, DeNoticia \
							  ...
							  ,CuotaExtraordinaria,CuotaExtraUnidad, Abono
...
# INICIO DEUDA
def Deudaxrecibo(recibo):# obtiene la deuda total de un recibo, incluyendo las ante
	listafacturas=[]
	...
	facturas = Recibo.objects.exclude(id=recibo.id).filter(Q(estado=0,unidad= recibo.unidad, fecha_generacion__lte=fechalimite)|\
			Q(estado=2,unidad= recibo.unidad, fecha_generacion__lte=fechalimite))
	# inicio metodo2
	deuda = 0
	cantidad = facturas.count()
	for factura in facturas :
		deuda = deuda + Valordeudarecibo(factura)
	if deuda == 0 or cantidad == 0:
		return listafacturas
	listafacturas.append(deuda)
	listafacturas.append(cantidad)
	return listafacturas
	# fin metodo 2
	...
	def get_context_data(self, **kwargs):
		...
		if EsMiPropiedad(uid,idpro):
			...
			recibos = Recibo.objects.filter(Q(estado=0\
											,unidad__propiedad = propiedad[0].idlegal)|\
											Q(estado=2,unidad__propiedad = propiedad[0].idlegal))\
											.order_by('unidad_id')
	...
	for recibo in recibos:
		...
		if recibosAntyAct:
			for factura in recibosAntyAct:
				...
				costo = costo + Valordeudarecibo(factura)
...
# INICIO crear pdf factura (la que se entregara a los propietarios):
# def facturapdf