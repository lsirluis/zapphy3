...
# inicio pruebas
from apps.pagos.views import listaReciboCosto, Deudaxrecibo
...
def apdf(request, id_propiedad, pk,factura):
	context={}
	if request.method == 'GET':
		...
		facturatemp = qs[0].idRecibo
		abono = total -  Valordeudarecibo(facturatemp)
		total = total - abono
		context['abono'] = abono 
		context['total'] = total
		context['total'] = total
		...
		# sino hay detalles
		if not qs :
			messages.add_message(request, messages.ERROR, 'Lo Sentimos no hay detalles para esta factura')
		# si hay detalles calculamos su deuda
		else:
			recibo= Recibo.objects.filter(id=factura)
			if recibo:
				context['deudas']=Deudaxrecibo(recibo[0])
				if context['deudas']: # est para que cuando no haigan deudas no de error
					context['total'] += context['deudas'][0]
		...
		return PDFTemplateResponse(request,"Pagos/testdetallepdf.html",context) #, filename='mypdf.pdf')
	else:
...
def to_pdf(request):
	...
	context['a']="hola"
	context['b']="luis"
		# context['filename']='my_pdf.pdf'
		# return PDFTemplateResponse(request,"Pagos/apdf.html",context) #, filename='mypdf.pdf')	
	return PDFTemplateResponse(request,"Pagos/testdetallepdf.html",context) #, filename='mypdf.pdf')
...
def pruebas(request):
	return render(request, "Pagos/testdetallepdf.html","")
...
def Addpago(request): # agregar if es ajax
	...
	# a sacar el costo total de la deuda conjunta de todos los recibos
				deuda=0 #deuda general de todos los recibos juntos
				...
				if valor == deuda:
					...
					response_data['errors'] = {"recibo":" el valor pagado es superior a la deuda, corrigalo!"}
				else : 
			...
			# el primer recibo
				for recibo in recibos:
					if valor >= Valordeudarecibo(recibo): # es un pago
						...
						RyC.save()
						AddAbono(recibo, nuevovalor, valor)
...
def Valordeudarecibo(recibo):
	# este metodo buscara las deudas de X recibo, pero hay diferencias 
	# en cuanto a recibos sin pagar y a recibos que han sido abonados
	# los recibos abonados su valor en deuda es realmente almacenado 
	# en la tabla abonos, esto con el fin de mantener los valores originales. intactos
	# para obtener la deuda de xrecibo, lo buscamos en la tabla abonos,
	# y sumamos cada abono realizado y al final lo restamos a la deuda listada normalmente
	# es por eso que primero preguntamos si el recibo esta abonado o en deuda total
	deuda = 0
	detalles = Detalle.objects.filter(idRecibo = recibo.id)
	for detalle in detalles:
		deuda = deuda + detalle.valor

	if recibo.estado == 0: # significa que tiene deuda y no es abono
		return deuda
	
	elif recibo.estado == 2: # si es un abono
		# en este punto tenemos la deuda original
		# ahora buscamos el valor total abonado 
		abonos = Abono.objects.filter(recibo=recibo)
		SaldoAbonado = 0
		for abono in abonos: # por cada abono que se encontro, se toma su valor
			SaldoAbonado = SaldoAbonado + abono.valor_abonado
		# en este punto tenemos la deuda original y el total de lo abonado
		# ahora haremos la resta, para obtener el valor adeudado
		valordeuda = deuda - SaldoAbonado
		return valordeuda
...
def AddAbono(recibo,inicial, abonado):
	restante = inicial - abonado
	abono = Abono( recibo = recibo, \
...
class listarList(ListView):
	...
	def get_context_data(self, **kwargs):
		...
		try:
			...
			context['object_list2'] = p.page(page)

		except PageNotAnInteger:
        # If page is not an integer, deliver first page.
			context['object_list'] = p.page(1)
			context['object_list2'] = p.page(1)

		except EmptyPage:
        # If page is out of range (e.g. 9999), deliver last page of results.
			context['object_list'] = p.page(p.num_pages)
			context['object_list2'] = p.page(p.num_pages)
		...
		cantidad = cantunidades(context['object_list'])
		temp = zip_longest(context['object_list'],cantidad,"")
		context['object_list']= temp
		

		return context
	# paginate_by = 1	
		# return contexto
def cantunidades(propiedades):
	listacantidad=[]
	for propiedad in propiedades:
		cantidad = Unidad.objects.filter(propiedad=propiedad.idlegal).count()
...
def apartamentos(request,id_propiedad):
	...
	if id_propiedad != "TOTALUNIDADES":
		...
		if Propiedad.objects.filter(administrador__idu = uid, idlegal=id_propiedad):
			...
			if apartamento :
				...
				for contador in apartamento2:
					if contador[0].valor_mora == 0 and contador[1] == False:
	...
	contexto['cantaldia'] = cantaldia
	contexto['cantendeudas'] = cantendeudas
	if V :
		contexto['busqueda'] = V
	if url:
